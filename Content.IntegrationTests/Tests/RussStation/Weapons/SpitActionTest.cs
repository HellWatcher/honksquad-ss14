...
public sealed class SpitActionTest : GameTest
{
    ...
    public async Task SpitActionIsWiredToProjectileSpell()
    {
        ...
        {
            Assert.That(protoManager.TryIndex<EntityPrototype>("ActionSpit", out var action), Is.True,
                "ActionSpit prototype should exist");
            Assert.That(protoManager.HasIndex<EntityPrototype>("ProjectileSpit"), Is.True,
                "ProjectileSpit prototype should exist");

            // The gun entity is gone; spit is now pure YAML on top of upstream's magic system.
            Assert.That(protoManager.HasIndex<EntityPrototype>("SpitGun"), Is.False,
                "SpitGun should have been removed by the ProjectileSpell migration");

            Assert.That(action!.TryGetComponent<WorldTargetActionComponent>(out var worldTarget), Is.True,
            ...
            Assert.That(action.TryGetComponent<ActionComponent>(out var actionComp), Is.True);
            ...
            Assert.That(action.TryGetComponent<PopupOnActionComponent>(out var popup), Is.True,
    ...
    public async Task HumanoidsAreGrantedSpit()
    {
        ...
        await server.WaitAssertion(() =>
        {
            Assert.That(protoManager.TryIndex<EntityPrototype>("BaseSpeciesAppearance", out var species), Is.True,
                "BaseSpeciesAppearance prototype should exist");
            Assert.That(species!.TryGetComponent<ActionGrantComponent>(out var grant), Is.True,
