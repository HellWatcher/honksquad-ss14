...
public sealed class VendingMachineBoundUserInterface(EntityUid owner, Enum uiKey) : BoundUserInterface(owner, uiKey)
{
    ...
    public void UpdateAmounts()
    {
        ...
        _cachedInventory = system.GetAllInventory(Owner);
        //HONK START - Pass item prices to vending UI
        var updatePrices = EntMan.TryGetComponent(Owner, out VendingPricesComponent? updatePricesComp)
            ? updatePricesComp.Prices
            : null;
        _menu?.UpdateAmounts(_cachedInventory, enabled, updatePrices);
        //HONK END
    }
